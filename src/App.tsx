...
const Sparkles = (props) => (
  <IconBase {...props}><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L12 3Z" /><path d="M5 3v4" /><path d="M3 5h4" /></IconBase>
);
const Bot = (props) => (
...
export default function App() {
  const [photoLeft, setPhotoLeft] = useState('/groom.jpg');
  const [photoRight, setPhotoRight] = useState('/bride.jpg');
  ...
  const handlePhotoUpload = (event, setPhotoFunc) => {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onloadend = () => {
        setPhotoFunc(reader.result);
      };
  ...
  return (
    // ★★★ 關鍵修改：items-start + py-10 讓手機版不被切掉 ★★★
